...
// const Services = () => {
//   // Retrieve token from localStorage as requested
//   const getAuthToken = () => localStorage.getItem('access_token_vendor') || '';
...
//   // Dynamic Header Utility including access_token_vendor
//   const getHeaders = () => {
...
//   // 1. Initial Load: Fetch Pricing Table & Categories API
//   useEffect(() => {
//     fetchInitialData();
//   }, []);
...
//     setLoading(true);
//     try {
//       await Promise.all([fetchPricingList(), fetchCategories()]);
//     } catch (err) {
...
//     try {
//       const res = await fetch('http://127.0.0.1:8000/api/professionals/vendor/services-pricing/', {
//         headers: getHeaders()
//       });
...
//     try {
//       const res = await fetch('http://127.0.0.1:8000/api/professionals/vendor/services/categories/', {
//         headers: getHeaders()
//       });
//       const result = await res.json();
//       if (result.status === 'success') {
...
//   // POST: Add Working Area
//   const handleAddArea = async () => {
//     if (!newAreaInput.trim()) return;
//     try {
//       const res = await fetch('http://127.0.0.1:8000/api/professionals/vendor/areas/', {
//         method: 'POST',
//         headers: getHeaders(),
//         body: JSON.stringify({ area_name: newAreaInput.trim() })
//       });
//       const result = await res.json();
//       if (result.status === 'success') {
//         setNewAreaInput('');
//         setShowAddAreaInput(false);
//         fetchPricingList(); // Refresh list to update area columns
//       }
//     } catch (error) {
...
//   // DELETE: Remove Working Area
//   const handleDeleteArea = async (areaName) => {
//     if (!window.confirm(`Are you sure you want to remove ${areaName}?`)) return;
//     try {
//       const res = await fetch(`http://127.0.0.1:8000/api/professionals/vendor/areas/${encodeURIComponent(areaName)}/`, {
//         method: 'DELETE',
//         headers: getHeaders()
//       });
...
//     // Filter available sub-categories matching backend category
...
//     try {
//       const res = await fetch(`http://127.0.0.1:8000/api/professionals/vendor/services/${item.offering_id}/edit/`, {
//         method: 'POST',
//         headers: getHeaders(),
//         body: JSON.stringify({
...
//     // Prepare clean area prices mapping
...
//       ? `http://127.0.0.1:8000/api/professionals/vendor/services/${editOfferingId}/edit/`
//       : 'http://127.0.0.1:8000/api/professionals/vendor/services/add/';
...
//   // Get dynamic unique categories from fetched pricing data
...
//   // Dynamic grid column template style
...
//             className="px-[16px] py-[8px] bg-gradient-to-r from-[#D61CA8] to-[#8B2EF5] rounded-[9px] text-[12px] font-bold text-white shadow-sm hover:opacity-95 transition-all"
//           >
//             + Add Service
//           </button>
//         </div>
...
//             className={`px-[15px] py-[6px] rounded-full text-[12px] transition-all ${
//               activeCategory === cat
...
//           {/* Table Body */}
//           {loading ? (
//             <div className="p-8 text-center text-[13px] font-semibold text-[#9090A0]">Loading services...</div>
...
//                     className="px-[10px] py-[5px] bg-[#D61CA814] rounded-[7px] text-[10px] font-semibold text-[#D61CA8] hover:bg-[#D61CA825]"
//                   >
//                     Edit
//                   </button>
//                   <button 
//                     onClick={() => handleToggleStatus(s)}
//                     className={`px-[9px] py-[5px] rounded-[7px] text-[10px] font-bold ${s.status === 'active' ? 'bg-[#F0F0F4] text-[#9090A0]' : 'bg-[#D1FAE5] text-[#059669]'}`}
//                   >
//                     {s.status === 'active' ? '⏸' : '▶'}
//                   </button>
//                 </div>
...
//               className="ml-1 text-[11px] text-[#9090A0] hover:text-red-500 font-bold"
//             >
//               ✕
//             </button>
//           </div>
...
//               className="px-[12px] py-[5px] bg-white border border-[#D61CA8] rounded-full text-[12px] font-semibold focus:outline-none"
//             />
//             <button
//               onClick={handleAddArea}
//               className="px-[12px] py-[6px] bg-[#D61CA8] text-white rounded-full text-[11px] font-bold"
//             >
//               Add
//             </button>
//             <button
//               onClick={() => setShowAddAreaInput(false)}
//               className="text-[#9090A0] text-[12px] font-bold px-1"
//             >
//               ✕
//             </button>
//           </div>
...
//             className="px-[14px] py-[7px] bg-[#F8F8FA] border-[1.5px] border-dashed border-[#C0C0CC] rounded-full text-[12px] font-semibold text-[#9090A0] hover:bg-[#EBEBEF]"
//           >
//             + Add Area
//           </button>
//         )}
...
//                 className="w-8 h-8 rounded-full bg-white border border-[#EBEBEF] text-[#9090A0] flex items-center justify-center font-bold text-[14px] hover:bg-[#F4F5F8]"
//               >
//                 ✕
//               </button>
//             </div>
...
//                   type="button"
//                   onClick={() => setIsAddModalOpen(false)}
//                   className="px-[16px] py-[9px] bg-[#F4F5F8] rounded-[9px] text-[12px] font-semibold text-[#9090A0] hover:bg-[#EBEBEF]"
//                 >
//                   Cancel
//                 </button>
//                 <button
//                   type="submit"
//                   className="px-[18px] py-[9px] bg-gradient-to-r from-[#D61CA8] to-[#8B2EF5] rounded-[9px] text-[12px] font-bold text-white shadow-sm hover:opacity-95"
//                 >
//                   Save Service
//                 </button>
//               </div>
...
const Services = () => {
  ...
  const handleDeleteArea = async (areaName) => {
    if (!window.confirm(`Are you sure you want to remove ${areaName}?`)) return;
    try {
      await deleteData(`/professionals/vendor/areas/${encodeURIComponent(areaName)}/`);
      toast.success(`${areaName} removed successfully`);
      fetchPricingList();
    } catch (error) {
      console.error('Failed to delete area:', error);
    ...
    <div className="flex-1 overflow-y-auto bg-[#F4F5F8] p-[24px]">
      ...
      <div className="bg-white rounded-[14px] overflow-x-auto shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
        <div className="min-w-[700px]">
          ...
          {/* Table Body with Shimmer Skeleton */}
          {loading ? (
            <TableSkeleton />
          ) : filteredServices.length === 0 ? (
            <div className="p-8 text-center text-[13px] font-semibold text-[#9090A0]">No services found.</div>
      ...
      <div className="flex flex-wrap gap-[8px] items-center">
        ...
        {showAddAreaInput ? (
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={handleAddArea}
              className="px-[12px] py-[6px] bg-[#D61CA8] text-white rounded-full text-[11px] font-bold cursor-pointer"
            >
              Add
            </button>
            <button
              onClick={() => setShowAddAreaInput(false)}
              className="text-[#9090A0] text-[12px] font-bold px-1 cursor-pointer"
            >
              ✕
            </button>
          </div>
